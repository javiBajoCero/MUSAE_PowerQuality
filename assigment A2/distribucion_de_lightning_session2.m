...
figure();
plot(d(:,1),data(:,5),'b. '); % doesnt make a lot of sense because also depends of DISTANCE so , if there is a grat induced overvoltage it could be that its just too close to the power line
xlabel('distance ')
ylabel('overvoltage (KW)')

figure();
histogram(data(:,5));
title('overvoltage');
xlabel('overvoltage (KW)')
ylabel('number of events')

figure();
plot(data(:,3),data(:,5),'b. ');
title('Current vs overvoltage');
xlabel('current of lighning (KA)')
